...
import { z } from "zod"
import { CalendarIcon, Loader2, BedDouble, PlusCircle, Trash2, FilePlus, History, RefreshCw } from "lucide-react"
...
const bookingFormSchema = z.object({
    ...
    channel_booking_id: z.string().optional(),
...
export function BookingForm({ isEditMode = false, bookingId }) {
    ...
    const roomRate = useMemo(() => {
        ...
        // If the user manually modified the rate (it differs from the base raw_room_rate), use their manual per-night multiplier
        const rawRate = form.getValues('raw_room_rate');
        if (watchedRoomRateModifier !== rawRate || !selectedRoomTypeId) {
            return (watchedRoomRateModifier || 0) * numberOfNights;
        }
        ...
        let total = 0;
        let currentDate = new Date(checkInDate); // Clone to avoid mutation
        ...
        while (isBefore(currentDate, checkOutDate)) {
            ...
            const priceForNight = isWeekend && selectedRoomType.weekend_price ? Number(selectedRoomType.weekend_price) : Number(selectedRoomType.weekday_price);

            total += (priceForNight || 0);
            currentDate = addDays(currentDate, 1);
        }
        return total;
    }, [checkInDate, checkOutDate, selectedRoomTypeId, roomTypes, watchedRoomRateModifier, numberOfNights, form]);
            ...
            async function fetchEditData() {
                try {
                    ...
                    const formattedBookingData = {
                        ...
                        charges: allCharges,
                        channel_source: bookingData.channel_source || '',
                        channel_booking_id: bookingData.channel_booking_id || '',
                    };
        ...
        <>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                            <FormField control={form.control} name="raw_room_rate" render={({ field }) => {
                                                const selectedRoomType = roomTypes.find(rt => rt.id === Number(selectedRoomTypeId));
                                                const hasWeekendPrice = selectedRoomType && selectedRoomType.weekend_price && selectedRoomType.weekend_price !== selectedRoomType.weekday_price;
                                                return (
                                                    <FormItem>
                                                        ...
                                                        {hasWeekendPrice && (
                                                            <FormDescription className="text-xs text-muted-foreground">
                                                                * Note: Friday & Saturday nights are billed at MYR {Number(selectedRoomType.weekend_price).toFixed(2)}. This will be calculated automatically in the totals.
                                                            </FormDescription>
                                                        )}
                                                        <FormMessage />
                                                    </FormItem>
                                        ...
                                        {form.getValues('channel_source') && (
                                            <div className="p-3 bg-slate-50 border rounded-md border-slate-200">
                                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Beds24 / External Source Info</p>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <p className="text-[10px] text-muted-foreground">Original Channel</p>
                                                        <p className="text-sm font-medium">{form.getValues('channel_source')}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] text-muted-foreground">Channel Booking ID</p>
                                                        <p className="text-sm font-medium">{form.getValues('channel_booking_id') || 'N/A'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                    ...
                    <div className="lg:col-span-1">
                        {isEditMode && bookingDataForLog && (
                            <Card className="mb-8">
                                <CardHeader className="pb-4">
                                    <CardTitle className="text-lg">Reservation Info</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-muted-foreground text-xs font-medium uppercase tracking-wider">Confirmation Code</span>
                                        <span className="font-bold text-primary text-base">{bookingDataForLog.confirmation_code || "-"}</span>
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <span className="text-muted-foreground text-xs font-medium uppercase tracking-wider">Platform</span>
                                        <span className="font-medium text-slate-700">{bookingDataForLog.channel?.name || bookingDataForLog.booking_source || "Direct"}</span>
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <span className="text-muted-foreground text-xs font-medium uppercase tracking-wider">Contact Number</span>
                                        <a target="_blank" rel="noopener noreferrer" href={`https://wa.me/${(bookingDataForLog.guest?.phone_number || '').replace(/\D/g, '')}`} className="flex items-center gap-2 text-green-600 hover:text-green-700 hover:underline font-medium mt-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" className="w-5 h-5 fill-current"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157.1zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z" /></svg>
                                            {bookingDataForLog.guest?.phone_number || "No Contact Number"}
                                        </a>
                                    </div>
                                </CardContent>
                            </Card>
